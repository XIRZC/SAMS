...
int scoreInsert(void)
{
	...
	if(hasFileAdded(fileName))
	{
		...
		for(i=0;i<count;i++)
		{
			if(stus[i].ID==temp.ID)
			{
				printf("在该专业下该年级下该学号对应的学生数据已存在，请重新输入！！\n");
				return 0;
			}
		} 
		count++;
...
void order(void)
{
	...
	printf(
		...
		"8.  设置当前文件存取数据的文件类型\n"
		"9.  打开某指定专业存储数据文件\n"
		"注意事项：插入为自动读写操作，编辑、删除、筛选、查看、排序等功能依赖于当前打开的\"major+grade\"文件\n" 
		"------------------------------------------\n");
	printf("请输入要进行的操作序号：");
	return;
}
