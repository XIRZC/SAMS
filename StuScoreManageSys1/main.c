		...
		switch (choice)
		{
			case 1:
				...
				if(byName==2)
				{
					...
					for(i=1;i<=Num[0];i++)
					{
						printf("%-14lld%-12s%-8s%-8d%-8.2f%-8.2f%-8.2f%-8.2f%-8.5f%-10d\n",stus[Num[i]].ID, stus[Num[i]].name, stus[Num[i]].sex, stus[Num[i]].grade, stus[Num[i]].math,
					stus[Num[i]].english, stus[Num[i]].sports, stus[Num[i]].cScore, stus[Num[i]].GPA, stus[Num[i]].rank);
					}
				}
				else if(byName==1)
				{
